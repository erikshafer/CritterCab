...
- **v0.3** (2026-05-19): Moment 2's happy path implemented per [`docs/prompts/implementations/003-dispatch-slice-5-2-fare-quoted-happy-path.md`](../prompts/implementations/003-dispatch-slice-5-2-fare-quoted-happy-path.md). W001 §5.2's happy-path GWT has runnable Alba coverage; `FareQuoteAutomation` reacts to `RideRequested` via Wolverine event-forwarding and records `FareQuoted` ($21.50 STANDARD-class canned response mirroring this Moment's narration). The three FareQuoteFailed alternate paths (*Transient failure with retry recovery*, *Exhausted retries*, *Non-transient failure*) defer to a follow-up slice 5.2 session that adds retry budget tracking via the `FareQuoteAttempts` projection. The v0.2 slot is deliberately unclaimed and remains available for a retroactive entry covering slice 5.1's implementation (PR #4), which predates the spec-delta closure-loop convention encoded in PR #18.
- **v0.4** (2026-05-19): Slice 5.2's three FareQuoteFailed alternate paths implemented per [`docs/prompts/implementations/004-dispatch-slice-5-2-fare-quoted-failure-paths.md`](../prompts/implementations/004-dispatch-slice-5-2-fare-quoted-failure-paths.md). All four W001 §5.2 GWTs (happy + transient-recovery + exhausted-retries + non-transient) now have runnable Alba coverage. Moment 2's narrative-prose scope is **unchanged** — failure paths remain deferred to a future failure-arc narrative per the *Pricing fails to quote* item in §`Deferred from this narrative`. What changed below the prose: `FareQuoteFailed` event + `FareQuoteAttempts` terminal-events-only projection are live; `FareQuoteAutomation` now runs a 3-attempt retry loop with a 2-second cooldown (test override 10ms); `RequestTimeline` folds both terminal outcomes. The *Pricing fails to quote* deferred item is now **implementation-complete**; only its narrative-prose treatment continues to defer. Retry-budget configuration remains hardcoded; `DispatchPolicy` view + `DispatchPolicyConfigured` event still defer to slice 11.
