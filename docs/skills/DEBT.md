...
- **Two design questions the reference impl must RESOLVE (not just describe) before it's enshrined** — surfaced by the slice-1 critter-skill-auditor Phase 2 audit:
  1. **ADR-011 Option A vs B for the Marten idiom.** W006 §6.1 locks "Option A (migration-time seed)", but `IInitialData` via `.InitializeWith<T>()` runs at *host startup* (Option B's timing) — it realizes Option A's intent only when driven by the JasperFx `resources setup` deploy step, and otherwise carries Option B's multi-instance seed race (benign under the idempotent guard + full-replacement). **Needs an ADR-011 amendment** describing the Marten realization and how it reconciles A/B, before a skill enshrines the wrong mechanism as canonical.
