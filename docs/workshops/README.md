...
| Identity BC's outbound `identity.driver-registered` contract (W003 Story 1 step 4) — companion to the existing rider-side `identity.rider-registered` forward-constraint from W002 §13 #2. | **done (modeled) / pending (authorship)** — W004 Slice 6.1 commits the receiver-side contract shape and generates the forward-constraint (W004 §X+1 #1–#3); `DriverRegistered` proto authorship remains pending the Identity workshop. |
