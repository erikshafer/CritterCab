...
**Status:** Complete (v0.11, 2026-05-09). All 12 slices walked and committed; retrospective closes the session.
**Started:** 2026-05-09.
...
1. **Workshop 001 §5.12 enum gap — `RIDER_NO_SHOW`.** Workshop 001's preferred `TerminationReason` enum has `DRIVER_NO_SHOW` and `PICKUP_ABANDONED` but no value for rider no-show specifically. Trips' `TripAbandonedAsNoShow` (slice 6.4) maps cleanest to a new `RIDER_NO_SHOW` value. **Disposition:** Workshop 001 revision needed; add `RIDER_NO_SHOW` to §5.12's preferred enum.
2. **Workshop 001 §5.12 enum gap — `ASSIGNMENT_COMPLETED_NORMALLY`.** Workshop 001's enum is silent on the happy-path completion case. Either add a value, or accept that Dispatch only records non-success outcomes (happy path implicit). **Disposition:** Workshop 001 revision call; choose explicit-value vs. implicit-happy-path.
...
1. **Driver-app projection timing budget.** A non-functional cross-cutting requirement: the driver-app trip-mode transition (from offer-mode to trip-mode following Dispatch's `OfferAccepted` → Trips' `TripMatched`) must complete inside a "subjectively instantaneous" budget. Achievable with current tooling (inline projection + ASB low-latency mode + gRPC server-streaming push). Worth documenting as an ADR so the budget becomes measurable (via `MatchingLatencyMetrics` projection per slice 6.1) and survives non-functional pressure changes. **Trigger fired at slice 6.1**; defer authorship per Workshop 001 §12.6 #4.

**Inherited from Workshop 001 §11 (status updated at session close):**

- ADR-candidate #5 — Configuration-as-events bootstrap strategy. **Fired this workshop (slice 6.11).** Trips' `TripsPolicyConfigured` is the second BC adoption; pattern confirmed across two BCs. Recommendation: lift to authored ADR.
- ADR-candidate #6 — Aggregate-per-invariant pattern. **Fired this workshop (§3 sidebar).** Recommendation: lift to authored ADR.
- ADR-candidate #7 — Shared ride identifier across BCs. **Fired this workshop (§3 + slice 6.1).** Recommendation: lift to authored ADR alongside #6.
- ADR-candidate #8 — ASB topic naming convention. **Fired this workshop (slice 6.9).** Two BCs now consistently apply the `<source-bc>.<event-name-kebab>` convention. Recommendation: lift to authored ADR.
...
- **Workshop 001 §5.12 revision** — add `RIDER_NO_SHOW` to preferred `TerminationReason` enum; possibly add `ASSIGNMENT_COMPLETED_NORMALLY`. Two parking-lot items.
