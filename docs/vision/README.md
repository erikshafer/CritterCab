...
**Telemetry.** High-volume GPS ingest and location-of-record for active drivers. Supplies driver location to Dispatch as a high-volume Kafka stream; Dispatch maintains its own local candidate projection from it ([ADR-018](../decisions/018-candidate-projection-ownership-and-telemetry-geospatial-supply.md) — overrides the earlier "geospatial index that Dispatch queries" framing). This is the Kafka-heavy context. Upstream of Dispatch; has no opinions about trips or business logic.
