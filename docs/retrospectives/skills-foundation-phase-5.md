...
| # | ai-skills skill | Drift observation | Surfaced during |
|---|---|---|---|
| 1 | `marten-advanced-dynamic-consistency-boundary` | Skill claims **"DCB is currently implemented in Polecat. The `[BoundaryModel]` attribute and `EventTagQuery` are Polecat-specific."** This contradicts both the file naming (Marten-prefixed) and the demonstrable presence of Marten DCB (`Marten.Events.Dcb.DcbConcurrencyException`, `opts.Events.RegisterTagType<>().ForAggregate<>()` on Marten store options, the `src/Persistence/MartenTests/Dcb/` test directory in the Wolverine repo). Likely written when DCB was Polecat-only and not updated when Marten gained the implementation. | Skill 14 (`dynamic-consistency-boundary`) audit |
...
- Skills reconciled: 14 / 39
- Total lines trimmed: ~462
- Direct-equivalent (deduplicated): 11
...
- Upstream-contribution candidates: 23 (21 footgun-style additions + 1 entire-skill-creation covering both gRPC skills (Active, Erik's roadmap) + 1 entire-skill-creation for sagas (Observed gap, no active author)). **Skill 14 alone contributed 7 candidates — the most footgun-rich skill in Phase 5.**
- Upstream-replacement candidates: 0
- Cab coverage gaps revealed: 6 (messaging resiliency + 3 from Skill 11: event enrichment, composite projections, RaiseSideEffects + 1 from Skill 12: Marten index design + 1 from Skill 13: operational daemon surface)
- Cab content corrections: 1 (MT/NSB interop section removed from `wolverine-azure-service-bus`)
- ai-skills content drift observed: 1 (Skill 14 — DCB skill claims DCB is Polecat-only, contradicting both naming and current Marten implementation)
- Forward-looking placeholders removed: 8 (Skills 4, 5, 8, 9, 11, 12, 13, 14 — pre-Phase-5 Cab pattern of placeholdering not-yet-published ai-skills counterparts; same nonexistent `marten-event-sourcing-fundamentals` referenced 5 times across Skills 9, 11, 12, 13, 14)
- Cab cleanup items deferred to post-Phase-5: 1 (Skill 10 rename)
- **Tier 1 complete (14 / 14)** — ready to begin Tier 2 (Polecat skills: `polecat-event-sourcing`, `polecat-document-store`)
