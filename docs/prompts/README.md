...
- [`workshops/004-onboarding-event-modeling.md`](./workshops/004-onboarding-event-modeling.md) — Author CritterCab's third Event Modeling workshop (fourth design-phase artifact overall) covering the Onboarding BC. **First EM workshop to consume a DS artifact as its primary upstream input** — replaces W002's narrative-pair input mechanism with W003's three-story spine plus 7 BC findings, 8 vocabulary items, 14 W004-scoped open questions, and 1 cross-cutting pattern. Closes the W003 §5.3 "Workshop 004 — Onboarding event model" follow-up; provides the third canonical data point for ADR-012 (aggregate-per-invariant) and ADR-013 (shared cross-BC identifier); third instance for ADR-011 and ADR-014; second instance for ADR-015. Scope locked at Tight (exact mirror of W003's three stories); re-application, multi-vehicle, and suspension lifecycle explicitly deferred. Sidebar lean: three aggregates (Application + AdjudicationCase + BackgroundCheckCase) — more aggressive plural-aggregate shape than W001's parent+sub-entity or W002's single-aggregate evidence. Introduces a new §X "DS findings handled" section adapted from W002 §13 "Forward-constraints handled" — convention for future DS-fed EM workshops. Expected new §11 ADR candidate: multi-vendor ACL pattern generalizing ADR-006. Status: authored (2026-05-26).
