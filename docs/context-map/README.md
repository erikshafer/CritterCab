...
flowchart LR
    ...
    Identity -->|"PL<br/>identity.*"| Trips
    Identity -.->|"PL<br/>(pending workshop)"| RiderProfile["Rider Profile"]
    Identity -.->|"PL<br/>(pending workshop)"| DriverProfile["Driver Profile"]
    Identity -.->|"PL<br/>(pending workshop)"| Onboarding
    ...
    Onboarding -.->|"<i>(expected CS)</i><br/>(pending workshop)"| DriverProfile
...
On the consumer-facing side, Identity publishes domain events to ASB under the [`identity.*` topic convention](../decisions/014-asb-topic-naming-convention.md) — concretely, `identity.rider-registered` and `identity.rider-profile-updated` are required publications per [W002 §13 forward-constraint #2](../workshops/002-trips-event-model.md), generated by Trips' need to surface rider name to driver post-acceptance. These are presumed-PL because the convention is locked, the contract artifacts are committed-in-principle, but the actual proto authorship will land at Identity's workshop. The relationship is PL rather than customer-supplier because each consumer absorbs Identity's vocabulary at its own boundary; Identity is not shaping its publications to any individual consumer's needs.
...
|---|---|---|
| Trips | Concrete (solid arrow) | [W002 §6.12](../workshops/002-trips-event-model.md) — `RiderProfileSnapshot` projection |
| Rider Profile | Presumed-PL (dashed arrow) | Pending Rider Profile workshop |
| Driver Profile | Presumed-PL (dashed arrow) | Pending Driver Profile workshop |
| Onboarding | Presumed-PL (dashed arrow) | Pending Onboarding workshop |
...
|---|---|---|
| Identity | All outbound edges to consumers — currently dashed except for Trips, which is locked via [W002 §6.12](../workshops/002-trips-event-model.md). The ACL stance toward Entra/OpenIddict is already locked by [ADR-006](../decisions/006-identity-provider-as-swappable-anti-corruption-layer.md) and survives unchanged through the workshop. | Strongest near-term candidate per the prompt's framing. The two outbound contracts required by [W002 §13 forward-constraint #2](../workshops/002-trips-event-model.md) (`identity.rider-registered`, `identity.rider-profile-updated`) will be the first concrete `identity.*` proto authorship. |
...
| Onboarding | Identity → Onboarding (edge #3); Onboarding → Driver Profile (edge #6). | Lifecycle BC; conversion of authenticated driver into Driver-Profile-active driver. **[W003 Domain Storytelling shipped 2026-05-26](../workshops/003-onboarding-domain-story.md)** — see §5.2 findings (especially B2 approval-terminal-gating on edge #6); event-model workshop (W004) remains pending. |
...
| **Trust & Safety** | Trips → Trust & Safety (edge #4). | **Not in the v0.1 vision-doc BC inventory.** Surfaced as a candidate BC by [W002 §10's](../workshops/002-trips-event-model.md) fan-out tables (consumer of `trips.trip-cancelled-by-driver` and `trips.trip-abandoned-as-no-show`). Inventory drift flagged for a follow-up vision-doc tidy session — the vision doc's BC inventory will need amendment when Trust & Safety's status firms up. |
